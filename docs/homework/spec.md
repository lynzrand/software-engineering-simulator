...
由于游戏的用户只有玩家一个人，在文章内用列表方式列举用例比绘制用例图更快捷方便。以下所有用例项目中都将直接使用列表展示用例，并省去说明中使用人员这一属性。
